...
def view_results(estimator, dataloader, option, modalities):
    ...
    for i, data in tqdm(enumerate(dataloader, 0), total=len(dataloader), desc=f'', unit='batch'):
        ...
        modelPoints = Variable(modelPoints).cuda()      # modelpoints_W
        ...
        rt_numpy = rt.squeeze(0).cpu().numpy()
        ...
        target_vs_pred(pc_depth_W, pc_velodyne_W, pc_model_W, modelPoints, pc_depth, pc_velodyne, pc_model, modelPointsGT, T, rt.detach().cpu().numpy())
...
def target_vs_pred(pc_depth_W, pc_velodyne_W, pc_model_W, modelPoints_W, pc_depth, pc_velodyne, pc_model, modelPoints, pred_RT, RT):
    ...
    pred_model_points = (pred_rotation_inv @ modelPoints_W.T).T + pred_translation_inv
    ...
    for i in range(1, len(modelPoints)):
        ...
        ax.plot(x1, y1, z1, color='red', linewidth=1)
        ax.plot(x_pred, y_pred, z_pred, color='blue', linewidth=1)
    ...
    ax.scatter(
        pred_pointcloud_vel[:, 0],
        pred_pointcloud_vel[:, 1],
        pred_pointcloud_vel[:, 2],
        s=1,
        alpha=0.5,
        color='magenta',
        label='pointcloud pred velodyne'
    )
    ...
    ax.scatter(
        pred_pointcloud_model[:, 0],
        pred_pointcloud_model[:, 1],
        pred_pointcloud_model[:, 2],
        s=1,
        alpha=0.5,
        color='red',
        label='pointcloud pred modelo'
    )"""
