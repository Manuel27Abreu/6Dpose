...
from datasets.PoseIndustrial6D.dataloader import PoseDataset2 as PoseDataset
...
def train_epoch(estimator, criterion, optimizer, dataloader, epoch, train_dis_avg, train_count, option, modalities):
    ...
    for i, data in tqdm(enumerate(dataloader, 0), total=len(dataloader), desc=f'Epoch {epoch}', unit='batch'):
        ...
        if (rt[0][0][3]**2 + rt[0][1][3]**2 + rt[0][2][3]**2) < 0.05:
            continue
        
        if math.sqrt(rt[0][0][3]**2 + rt[0][1][3]**2 + rt[0][2][3]**2) > 20:
            continue
        ...
        if option == 1:
            pred_r, pred_t, pred_c, _ = estimator(img, depth_vel*Depth2Enable, model*PC1Enable, velodyne*PC2Enable, choose, idx)
        elif option == 2:
            pred_r, pred_t, pred_c, _ = estimator(img, depth_vel*Depth2Enable, points*PC1Enable, velodyne*PC2Enable, choose, idx)
        elif option == 3:
            pred_r, pred_t, pred_c, _ = estimator(img, depth_vel*Depth2Enable, model*PC1Enable, points*PC2Enable, choose, idx)
        elif option == 4:
            pred_r, pred_t, pred_c, _ = estimator(img, depth_vel*Depth2Enable, model*PC1Enable, points*PC2Enable, choose, idx)
        elif option == 5:
            pred_r, pred_t, pred_c, _ = estimator(img, depth_vel*Depth2Enable, model*PC1Enable, velodyne*PC2Enable, choose, idx)

        # loss, dis, new_points, new_target = criterion(pred_r, pred_t, pred_c, target, points, idx, points, opt.w, opt.refine_start)               # com todos os pontos
        loss, dis, new_points, new_target = criterion(pred_r, pred_t, pred_c, modelPointsGT, modelPoints, idx, points, opt.w, opt.refine_start) # apenas com alguns pontos        
...
def save_real_vs_reconstruction(estimator, dataloader, option, modalities):
    ...
    R = torch.tensor([
        [1 - 2*(y**2 + z**2),     2*x*y - 2*z*w,        2*x*z + 2*y*w],
        [2*x*y + 2*z*w,           1 - 2*(x**2 + z**2),  2*y*z - 2*x*w],
        [2*x*z - 2*y*w,           2*x*w + 2*y*z,        1 - 2*(x**2 + y**2)]
    ], device='cuda:0')

    R_numpy = R.cpu().numpy()
    t_numpy = pred_t.detach().cpu().numpy()

    T = np.eye(4)
    T[:3, :3] = R_numpy
    T[:3, 3] = t_numpy

    target_vs_pred(pc_depth_W, pc_velodyne_W, pc_model_W, pc_depth, pc_velodyne, pc_model, T, rt.detach().cpu().numpy())
...
def main():
    ...
    option = 4
    if option == 1:
        opt.outf = 'trained_models/1-img_model-velodyne-pc_model-pc_velodyne'
        concat = "model"
        mask = "velodyne"
        print("option1- img_model-velodyne-pc_model-pc_velodyne")
    elif option == 2:
        opt.outf = 'trained_models/2-img_depth-velodyne-pc_depth-pc_velodyne'
        concat = "depth"
        mask = "velodyne"
        print("option2- img_depth-velodyne-pc_depth-pc_velodyne")
    elif option == 3:
        opt.outf = 'trained_models/3-img_model-depth-pc_model-pc_depth'
        concat = "model"
        mask = "depth"
        print("option3- img_model-depth-pc_model-pc_depth")
    elif option == 4:
        opt.outf = 'trained_models/4-img_depth-depth-pc_model-pc_velodyne'
        concat = "depth"
        mask = "depth"
        print("option4- img_depth-depth-pc_model-pc_velodyne")
    elif option == 5:
        opt.outf = 'trained_models/5-img_depth-velodyne-pc_model-pc_velodyne'
        concat = "depth"
        mask = "velodyne"
        ...
        concat = "depth"
        mask = "model"
        print("option6- img_depth-velodyne-pc_model-pc_velodyne")
    ...
    for epoch in range(opt.start_epoch, opt.nepoch+1):
        ...
        diz_loss['train_loss'].append(train_dis_avg / opt.batch_size)
        ...
        tqdm.write(f"Epoch {epoch}/{opt.nepoch+1}\t Train_dis: {train_dis_avg / opt.batch_size:.6f} \t Eval_dis: {test_dis:.6f}")
