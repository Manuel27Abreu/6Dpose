...
class PoseNetMultiCUSTOMPointsX(nn.Module):
    ...
    def forward(self, img, depth_vel, x,velodyne, choose, obj):
        ...
        # print("apx.shape", ap_x.shape)
        # print("apy.shape", ap_y.shape)

        ap_x = ap_x.permute(1, 0, 2) 
        ap_y = ap_y.permute(1, 0, 2) 

        # print("apx.shape apos permute", ap_x.shape)
        # print("apy.shape apos permute", ap_y.shape)
    
        ap_x = torch.cat([ap_x, ap_y], 1)
        ap_x = self.netFinal(F.dropout(ap_x, p=0.0025))

        # print("ap_x.shape apos netFinal", ap_x.shape)

        ap_x = ap_x.flatten()

        # print("ap_x.shape", ap_x.shape)
